...
1. **`findWhere`/`deleteWhere`/`db.<c>.filter(predicate)` empujados a SQL para CUALQUIER predicado, no solo `==`**: `countWhere`/`findWhere` ya empujan el caso `|x| x.campo == valor` a SQL de verdad (ver Hecho, v1.59.0) -- sigue pendiente: `deleteWhere` (todavía trae todo + borra fila por fila, aunque el predicado sea pusheable), y cualquier forma de predicado más allá de una sola igualdad (`>`/`<`/`!=`, `&&`/`||` combinando varias condiciones, comparar dos campos entre sí). Ítem grande (un mini compilador de predicado a SQL más completo, parametrizado de forma segura contra los dos backends) -- la ronda de v1.59.0 se acotó a propósito al caso de igualdad simple (el más común, y el que motivó el pedido: `reviews.productId`/`telemetry.sessionId`), el resto queda para una ronda dedicada. **Prioridad subida (24/08/2026), reforzado por un adoptador nuevo ("CRM"/Nexus, 11 `.link`, analizado por primera vez esta sesión)**: tres casos reales de ALTA FRECUENCIA (llamados en cada carga de página/UI, no en un backfill puntual) que `countWhere`/`findWhere` todavía no cubren -- `notifications.link` (`unreadCount()`, el badge de notificaciones: `n.userId == uid && !n.read`, dos condiciones), `inventory.link` (alerta de stock bajo: `p.stock <= 5 && p.stock > 0`), `chat.link` (contador de conversaciones sin leer: `c.unreadCount > 0`, un solo `>`). Los tres traen la colección ENTERA a memoria hoy. Candidato natural para la primera extensión: `>`/`<`/`!=` de un solo campo (mismo mecanismo que `==` de v1.59.0, solo cambia el operador SQL emitido), antes que `&&`/`||` compuesto (más grande, necesita combinar varias condiciones de forma segura).
