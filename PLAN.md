...
**`LSP completo` de esta misma fila: RESUELTO, Nivel 1+2.** Los 3 prerrequisitos (spans+diagnósticos, recuperación de errores del parser, spans en AST/checker) más el protocolo en sí (`linkc lsp` -- JSON-RPC sobre stdio, diagnósticos con imports resueltos de verdad vía `load_program_with_overlay`, hover/completion/goto-def a nivel de declaración) ya están -- ver GRAMMAR.md §3.19 para el diseño completo y qué queda explícitamente para un Nivel 3 futuro (completion sensible a posición tipo `x.<TAB>`, hover de una expresión arbitraria). También del "Backend a WASM" de Fase 1: el "codegen directo vía `wasm-encoder`" que esa fila nombraba como evolución ya existe en v0 (`linkc wasm`, GRAMMAR.md §3.20) -- alcance mínimo a propósito (solo aritmética entera sobre `Int`/`Bool`, una función es una sola expresión), el target `wasm32-wasip1` que recompila el intérprete entero sigue siendo el camino real de producción. `package manager` en red y `observabilidad` de fondo de esta misma fila siguen sin empezar (`link.lock`, GRAMMAR.md §2.1, es detección de deriva por hash, no resolución de dependencias con versiones) -- ver GRAMMAR.md §2.1 y el README para el detalle de qué falta y por qué se dejó afuera de esta ronda.
