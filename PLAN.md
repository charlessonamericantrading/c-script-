...
1. **`findWhere`/`deleteWhere`/`db.<c>.filter(predicate)` empujados a SQL para CUALQUIER predicado, no solo una conjunción `&&`**: `countWhere`/`findWhere` ya empujan a SQL de verdad tanto el caso de un solo operador (`==` v1.59.0, `!=`/`<`/`<=`/`>`/`>=` v1.71.0) como una conjunción `&&` de varias hojas simples (v1.72.0, ver Hecho -- los DOS casos reales de CRM que quedaban sin resolver, `notifications.link` e `inventory.link`, ya empujan). Sigue pendiente: `deleteWhere` (todavía trae todo + borra fila por fila, aunque el predicado sea pusheable), **`||` combinando condiciones** (sin evidencia real de demanda todavía, a diferencia de `&&`), y comparar dos campos del propio parámetro entre sí (`endDate > startDate`, tampoco cubierto).
2. **Índices/constraints ÚNICOS COMPUESTOS (de varios campos) declarativos**: `@index`/`@unique` de un solo campo ya está resuelto (ver Hecho, v1.45.0) -- la forma equivalente sobre VARIOS campos a la vez (`@unique(["email", "tenantId"])`, por ejemplo) sigue pendiente, necesitaría una anotación a nivel de `type`, no de campo (`TypeDecl` no tiene `annotations` hoy). **Ampliado (24/08/2026) por un caso real en Glowapp** (no usa c-script, pero su schema Drizzle es evidencia de demanda): `appointments` usa un índice único COMPUESTO Y CONDICIONAL a la vez -- `UNIQUE(userId, appointmentDate, startTime) WHERE status != 'cancelled'` -- permite reusar un horario una vez cancelado, sin acumular filas basura. Cuando se ataque este ítem, la forma condicional (`WHERE ...`) debería considerarse en el mismo diseño, no como una ronda aparte.
3. **Constraints `@check` declarativos MÁS ALLÁ de un rango numérico simple**: `@check(min/max/range, ...)` sobre `Int`/`Int64`/`Float` ya está resuelto (ver Hecho, v1.60.0) -- una expresión booleana arbitraria (comparar dos campos entre sí, ej. `endDate > startDate`) o un constraint sobre `String` (longitud, no-vacío) siguen sin forma de declararse.
4. **`--db-schema <nombre>` o `--db-prefix`**: namespacing para compartir una base Postgres entre varios `.link` sin pensar en colisiones de nombre. **Auditado (24/08/2026), deliberadamente diferido, no atacado esta ronda**: a diferencia del resto de ítems de esta lista (puntos de inserción limpios, riesgo acotado), este necesitaría enhebrar el prefijo/schema por TODOS los lugares que hoy arman SQL con el nombre de colección tal cual -- `create_table_sql`/`check_schema_matches`/`check_schema_for_adoption`/`select_rows`/`select_rows_page`/`select_rows_after`/`select_grouped`/`Db::call` entero (insert/applyPatch/delete/count/subscribe)/`count_where_equals`/`find_where_equals`/`warn_if_table_looks_unrelated`/`validate_existing_id_column`/`validate_columns_exist_for_adoption`/`create_index_statements`, más los equivalentes de `codegen/postgres_emit.rs` y `introspect.rs` -- decenas de sitios en 3 archivos, varios tocados en rondas MUY recientes de esta misma sesión (§3.94/§3.95/§3.96/§3.97). Forzarlo en la misma ronda que esos cambios habría sido el tipo de refactor grande y apurado que este proyecto evita a propósito -- queda para una ronda dedicada, con su propio tiempo de revisión.
5. **`@cache("60s")` declarativo** sobre un rpc, para lecturas costosas y poco cambiantes.
6. **Idempotency keys nativas** en rpcs de escritura: hoy hay que implementar la comprobación de "¿ya existe?" a mano antes de cada inserción en un backfill con reintentos. **Reforzado (24/08/2026) por Glowapp** (no usa c-script, pero varios de sus webhook handlers de pago -- `v2RevolutHandler.ts`, `orderConfirmation.ts`, `connections.ts` -- implementan a mano exactamente este chequeo de "¿ya procesé este evento?" antes de escribir, evidencia de demanda real y repetida, no un caso aislado).
...
6. **`@rate_limit` con una clave adicional a la IP** (ej. un campo del body/query, combinado con la IP): gap NUEVO (24/08/2026), encontrado analizando Glowapp (no usa c-script, pero su propio middleware de rate limit -- `v2RateLimit.ts` -- es evidencia de un caso de abuso real). Hoy `@rate_limit` (§3.39, con `--trust-proxy`/§3.89 para el caso detrás de proxy) solo puede tener como clave la IP -- ese middleware limita por `prefix:ip:email` en vez de solo IP, con un comentario propio explicando el motivo: un endpoint de pago tenía un campo de identidad distinto y con solo-IP alguien podía rotar de IP reusando el mismo email para seguir abusando. Candidato: una forma de nombrar un campo adicional del body/query como parte de la clave del límite, sin perder el comportamiento actual (solo-IP) como default.
...
1. **`smtp` con adjuntos y cc/bcc**: `sendToMany`/`sendHtml` ya resueltos (GRAMMAR.md §3.63); adjuntos y cc/bcc siguen sin cubrir. El módulo `storage`/S3 (§8.5) y el envío asíncrono de `smtp` (§8.3.3) ya están trackeados -- este ítem es específicamente lo que falta de `smtp.send` en sí. **Prioridad subida (24/08/2026), reforzado por "CRM"/Nexus** -- primer caso real de un adoptador ABANDONANDO por completo el módulo `smtp` en vez de trabajar alrededor de sus límites: `mailer.link` (`Mailer.send`) ni siquiera llama a `smtp.*` -- solo escribe una fila de log y marca `sent: true` sin enviar nada. El envío real ocurre en `server/services/email/` (314 líneas de TypeScript con `nodemailer`): adjuntos, rate-limiting configurable, tracking por destinatario, envío asíncrono. Evidencia de que este gap ya cuesta una salida completa del lenguaje para un módulo entero, no solo una fricción menor.

### 9.7 CLI y Experiencia de Desarrollador
1. **Unix domain sockets** (`--socket /run/app.sock`) como alternativa a TCP.
2. **Suite de administración de datos**: `linkc db inspect`/`db shell`/`export`/`import`/`seed` -- listar tablas y conteo de filas, REPL de solo lectura, exportar/importar entre entornos o motores, poblar una base nueva desde un fichero.
3. **RPCs de administración estándar opcionales** (`_admin.vacuum()`, `_admin.tableStats()`) detrás de `@requires(Role.Admin)`.
4. **`linkc systemd <archivo> <puerto>`**: generador de unidad systemd, a la par de `linkc docker` que ya existe.
5. **`linkc pm2-config <archivo> <puerto> -o ecosystem.json`**: generador de configuración PM2.
6. **Tareas programadas/recurrentes nativas (cron-like) dentro de `linkc serve`**: identificado y deliberadamente diferido en la ronda §8.4-8.5 (23/08/2026) por "menor demanda general" (ver nota al pie de esa sección). **Re-priorizado (24/08/2026) por evidencia fuerte de Glowapp** (no usa c-script, pero es la señal): 10+ schedulers hand-rolled con `setInterval` (`appointmentReminderScheduler.ts`, `abandonedCartScheduler.ts`, `automationScheduler.ts`, `enrichmentScheduler.ts`, `goalsRecalculator.ts`, etc.), más un supervisor completo (`schedulerSupervisor.ts`) con registro de jobs, guard contra solapamiento, arranque escalonado, y hasta un workaround para el límite de 32 bits de `setInterval` en intervalos largos. La demanda que faltaba para justificar esto ya no es hipotética.
...
1. **`sitemapXml`/`robotsTxt` declarativos**: hoy un `sitemap.xml` se escribe a mano armando el XML como `String` (ver el ejemplo de §3.35). Candidato: `sitemapXml(urls: {loc: String, lastmod: Timestamp?}[]) -> String` como builtin -- arma el XML bien formado, el rpc sigue siendo responsable de la lista de URLs (viene de la base, `@route` no puede inferir rutas dinámicas por sí solo). Mismo criterio de "helper que devuelve `String`" que `escapeHtml`, no un motor de templates nuevo.
2. **Metadata SEO clásica como helpers de `String`**: meta tags/Open Graph/canonical URL y JSON-LD (`schema.org`) como funciones builtin que arman un fragmento de `<head>` bien formado (`escapeHtml` interno para cualquier valor de usuario) -- para no tener que concatenar HTML a mano ni traer un motor de templates.
3. **`robots.txt` con reglas para crawlers de IA**: un preset/builtin declarativo cubriendo los user-agents de entrenamiento/búsqueda de IA conocidos (`GPTBot`, `ClaudeBot`, `PerplexityBot`, `Google-Extended`, etc.) además de las reglas clásicas -- para que un adoptador no tenga que mantener esa lista a mano y desactualizarse. Mismo mecanismo que el ítem 1 (`@content_type("text/plain")` + un `String` bien formado), solo con el contenido ya armado.
4. **`llms.txt` auto-generado por proyecto** (convención [llmstxt.org](https://llmstxt.org/), no confundir con el `llms.txt` de ESTE repo que documenta el compilador): `linkc build` emitiría un `llms.txt` junto a `contract.d.ts`/`openapi.json`, listando servicios/rpcs con sus docstrings `///` (§3.72) ya capturados -- para que un agente de IA externo que llega al proyecto entienda la API sin tener que leer el `.link` completo. Reusa infraestructura existente (captura de docstring), sin gramática nueva.
5. **Ejemplos estructurados por rpc en `openapi.json`**: más allá de la descripción de `///` (§3.72), una forma de declarar un ejemplo de request/response real -- ayuda a un agente que consume la API (o a un humano generando código desde el contrato) a entender la forma exacta sin adivinar a partir del tipo solo.
6. **`response.redirect(url, permanent: Bool)`**: azúcar sobre `response.setStatus` (§3.46) + header `Location` -- hoy un redirect 301/302 real necesitaría armar eso a mano si es que es posible con la API actual; canónico para SEO (evitar contenido duplicado, consolidar un dominio/URL viejo hacia el nuevo).
7. **`@cache_control("...")` por rpc**: header `Cache-Control` declarativo -- relacionado con `@cache` (§9.3.5, que es caché de SERVIDOR, ejecuta el rpc menos seguido) pero más simple: solo el header HTTP hacia afuera, para que un CDN o un crawler de IA sepa cuánto tiempo puede confiar en una respuesta sin volver a pedirla.

### 9.10 Diferido -- ya trackeado en §8, prioridad menor que 9.1–9.9
...
### 9.11 Bloqueado -- requiere algo externo o una decisión previa
