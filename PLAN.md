...
1. **`findWhere`/`deleteWhere`/`db.<c>.filter(predicate)` empujados a SQL para CUALQUIER predicado, no solo un único operador**: `countWhere`/`findWhere` ya empujan el caso `|x| x.campo OP valor` a SQL de verdad para los SEIS operadores de comparación -- `==` (v1.59.0), `!=`/`<`/`<=`/`>`/`>=` (v1.71.0, ver Hecho) -- sigue pendiente: `deleteWhere` (todavía trae todo + borra fila por fila, aunque el predicado sea pusheable), y **`&&`/`||` combinando VARIAS condiciones a la vez**, que sigue siendo el ítem grande de verdad -- un mini compilador de predicado a SQL parametrizado de forma segura contra los dos backends. **Dos de los tres casos reales de CRM que motivaron subir la prioridad de este ítem (24/08/2026) siguen sin resolver por esto**: `notifications.link` (`n.userId == uid && !n.read`) e `inventory.link` (`p.stock <= 5 && p.stock > 0`) combinan DOS condiciones -- solo `chat.link` (`c.unreadCount > 0`, un único operador) se benefició de la ronda de un solo operador. Comparar dos campos entre sí (`endDate > startDate`) tampoco está cubierto.
