...
`package manager` en red y `observabilidad` de fondo de esta misma fila siguen sin empezar (`link.lock`, GRAMMAR.md §2.1, es detección de deriva por hash, no resolución de dependencias con versiones) -- ver GRAMMAR.md §2.1 y el README para el detalle de qué falta y por qué se dejó afuera de esta ronda.
