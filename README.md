...
Not done yet: LSP Level 3, rounds 2/3 and 3/3 (hover of an arbitrary mid-body expression, then position-sensitive completion after `x.`, in that order — see GRAMMAR.md §3.21 for why), and closing the WASM native-codegen gap for real programs (statements, non-scalar types — see GRAMMAR.md §3.20). See [GRAMMAR.md](GRAMMAR.md) §2.1 for what's still missing in the import/package-manager story (dependency version resolution, a network registry).
