...
<div align="center">
  ...
  <p>
    ...
    <a href="https://github.com/charlessonamericantrading/c-script-/releases"><img src="https://img.shields.io/badge/version-1.149.0-blue.svg" alt="Version" /></a>
...
- `linkc doctor <file> [--db <url|file>] [--target-url <url>]`: environment diagnostics before a deployment — the `linkc` version, that the entry file resolves its imports/parses/type-checks, write permission in its directory, and read-only connectivity (`SELECT 1`, never any DDL) to the configured database. With `--target-url` (or `LINK_DOCTOR_TARGET_URL`), also hits `<url>/health` on an already-running `linkc serve` and flags a version mismatch between local and remote — the local dev machine drifting behind (or ahead of) production is otherwise a silent gap with no error message. Prints a checklist and exits `1` if any real check failed, meant for a CI gate before `linkc serve`
...
- Adopting a table with a native PostgreSQL `uuid` column (a `Uuid` or plain `String` field either way) or `inet`/`cidr` column (a `String` field) now reads and writes correctly — those three have their own binary wire format, not UTF-8 text, so mapping them to `String`/`Uuid` used to compile but fail on the first real row (`linkc introspect` already recommended `String` for `inet`; that recommendation now actually works). No new type — a `source_ip inet` column round-trips through a plain `source_ip: String?` field
- `linkc introspect <postgres-url>` generates a starting `.link` (types + `db {...}`) from an existing PostgreSQL database's schema — for adopting a system with real data instead of writing every field by hand. A starting point to review, not production-ready as-is: any column it can't map with confidence (`jsonb`, a bare `time` with no date) still gets a valid type (`String`) plus a warning on stderr, never silently dropped. A native `uuid` column maps to `Uuid`, and a native `inet`/`cidr` column to `String`, both exact mappings with no warning — reading/writing those columns is verified against real Postgres wire format, not just "compiles." A primary key literally named `id` emits `id: Int` for an integer column or `id: Uuid` for a native PostgreSQL `uuid` column, both with no warning — any other real type still gets the `id: Int` placeholder plus a warning, since neither shape is adoptable yet. PostgreSQL only, no `service` generated
- A collection's primary key can be `id: Uuid` instead of `id: Int` — generated application-side on every `insert` (same generator as `crypto.uuid()`), never relying on a column `DEFAULT`/`RETURNING`/`last_insert_rowid()`, which is what makes adopting an already-existing `id uuid` table (common when migrating from another backend) possible without touching its schema. `find`/`applyPatch`/`delete`/`increment`/`insert`/`upsert`/`page`/`maxRow`/`minRow` all work the same as with `id: Int`. `pageAfter` is rejected on a `Uuid`-keyed collection on purpose — its "never skips a concurrently-inserted row" guarantee depends on the id growing in insertion order, which a random UUID doesn't provide; use `page` (offset-based) instead. SQLite stores the column as `TEXT PRIMARY KEY NOT NULL`; PostgreSQL uses the native `UUID` column type, with hand-rolled binary encoding/decoding for that column (no new dependency) so it works against a real pre-existing `uuid` column too, not just one `linkc serve` created itself
...
- Per-client rate limiting: `@rate_limit("20/1m")` caps an rpc to N requests per time window per `(client IP, service, rpc)`, 429 on exceeding, token bucket with continuous refill. On PostgreSQL, the bucket is shared for real across every `linkc serve`/`serve-all` instance pointed at the same database (an internal table, invisible to `db {}`/`introspect`/`migrate`) — no more instances multiplying the effective limit; degrades to the old per-process behavior automatically if the table can't be created (`--adopt-existing` without it pre-created, or a role without `CREATE TABLE`). SQLite stays per-process — a single file/process already has the exact state in memory, nothing to distribute
...
- `link.json` git dependencies (`"name": "git+<url>#<tag-or-branch-or-commit>"`) now pin for real: after the first `linkc build`, `link.lock` records the resolved commit and every later build/serve/test reuses it exactly — a branch dependency no longer silently re-resolves (or, before this round, silently froze at the wrong commit due to a real staleness bug in how a branch ref was checked). `linkc build --update-deps` is the one command that re-resolves fresh and advances the pin, same split as `cargo update` vs. plain `cargo build`. Two concurrent `linkc build`/`serve` processes resolving the same dependency no longer race on the same local clone either (an advisory file lock, self-healing if a process dies mid-clone)
...
| Limitation | Detail |
|---|---|
| `@rate_limit` on SQLite is per-process, in-memory | No persistence across restarts, no coordination across replicas if the same `.link` runs on more than one process — see Works today for the PostgreSQL case, which now shares one real bucket across every instance pointed at the same database. |
...
| No CSP | Depends on each page's actual content — no way to get a safe default without it. (HSTS is available, opt-in: `--hsts <value>`/`LINK_HSTS`, since `linkc serve` itself never terminates TLS and can't send it by default — see Works today.) CORS allowlist entries are exact-match only, no wildcard subdomains. |
...
Link is verified by **1410 automated unit, integration and CLI tests**, including tests that
