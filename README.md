...
Not done yet: the LSP protocol server itself (see above — all three prerequisites are now in place, but the server hasn't been started). Also still missing: a `.wasm`-emitting codegen backend (today's WASM target recompiles the tree-walking interpreter rather than generating native wasm instructions), and a real SQL-backed DB. See [GRAMMAR.md](GRAMMAR.md) §2.1 (imports/package manager) and §3.12 (`db`) for exactly what each of those means and why.
