...
**Límite honesto, documentado en el barrido de "límites honestos" (26/08/2026): `crypto.hashPassword`/`verifyPassword` corren SINCRÓNICAMENTE sobre el único hilo del servidor.** El costo real de Argon2id (deliberado -- es justamente lo que lo hace resistente a fuerza bruta, GRAMMAR.md §3.34) es del orden de decenas de milisegundos con parámetros razonables, y `linkc serve` procesa una request a la vez (GRAMMAR.md §3.10) -- así que un `rpc login`/`register` bajo un pico REAL de tráfico (una campaña, un evento) encola cada hash detrás del anterior, bloqueando el proceso ENTERO mientras tanto: no solo las llamadas a `login`, sino CUALQUIER otro rpc, incluido `/health`. Peor todavía como vector de ataque DELIBERADO: alguien mandando requests de login concurrentes sin límite puede colgar el servidor entero con tráfico barato de generar, sin necesitar credenciales válidas ni explotar ningún bug -- el costo lo paga el propio servidor, por diseño de Argon2id.

**La mitigación ya existe en el lenguaje, solo no estaba conectada acá: `@rate_limit("N/window", key: <param>)` (GRAMMAR.md §3.39/§3.142) sobre el `rpc` que llama a `hashPassword`/`verifyPassword`.** Acotar cuántos logins/registros por ventana de tiempo acepta el servidor acota, de forma directa, cuántos hashes pueden estar encolados a la vez -- la defensa correcta contra el vector de ataque de arriba, no un apagado del problema sino la razón por la que ese flag existe. `--argon2-memory-kib`/`--argon2-iterations` más bajos (arriba) bajan el costo por hash a cambio de menos margen de seguridad -- un lever de operación, no un reemplazo de `@rate_limit`. Correr varios `linkc serve`/`serve-all` detrás de un balanceador (GRAMMAR.md §3.92) acota el radio de impacto por proceso, pero cada proceso individual sigue siendo single-threaded -- no es una solución, solo reduce cuánto tráfico concentrado hace falta para tumbar UN proceso puntual. Resolver esto del todo (hashing en un pool de hilos separado del intérprete) es un cambio de arquitectura real -- el intérprete entero asume dispatch estrictamente serial, `Value`/`Db` no son `Send` a propósito (§3.10) -- fuera de alcance de un fix chico; sin evidencia real de que haya sido explotado todavía, así que se documenta el límite y la mitigación disponible en vez de forzar un rediseño.
...
**`in_transaction` SOLO atrapa el anidamiento SINTÁCTICO -- y desde el 26/08/2026, el caso que se le escapa da un error claro en runtime, no uno crudo del backend.** Bug real, encontrado por una auditoría multi-agente adversarial, no por un reporte externo: `in_transaction` es un `Cell<bool>` con alcance de UN `check_block` -- sin visibilidad sobre lo que hace una `fn` auxiliar llamada desde adentro. Una `transaction` alcanzada por una llamada a otra función que a su vez abre su PROPIA `transaction` (anidamiento real, pero a través de un límite de función, no de sintaxis) compilaba limpio y recién fallaba en runtime con el error crudo del backend ("cannot start a transaction within a transaction"), sin ninguna pista de qué regla de c-script se estaba violando. `Db::begin_transaction` (`runtime/db.rs`) ahora chequea, ANTES de intentar el `BEGIN` real, si ya hay una transacción abierta en esta misma ejecución (el intérprete es single-threaded, así que esto nunca es un falso positivo entre requests distintas) -- mismo mensaje claro que el checker ya usa para el caso sintáctico. El límite de FONDO (el checker estructuralmente no puede atrapar esto en compilación, solo en runtime) queda igual -- ver el bullet de abajo.
...
**Verificado**: 2 tests unitarios nuevos con hilos de sistema operativo reales (`runtime/mod.rs`, 40 inserts concurrentes sin pérdida ni duplicado de id; 40 transacciones concurrentes sobre la misma fila dando exactamente 40) + 2 tests de compilación nuevos (`Db: Send + Sync`, `store.rs`; los tipos de conexión de `rusqlite`/`postgres` siguen siendo `Send`, invariante de la que depende toda esta arquitectura) + la suite completa existente (1213 tests) sin ninguna regresión, confirmando que el comportamiento de una sola request a la vez es byte-a-byte idéntico al de antes. Verificación manual descripta arriba contra `linkc serve` real, SQLite y Postgres.
