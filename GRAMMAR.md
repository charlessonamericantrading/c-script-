...
**El cast `::uuid` explícito que hace posible bindear contra una columna Postgres nativa sin la dependencia `with-uuid-1`.** `postgres` (la crate, sin ese feature) no tiene una encodificación binaria propia para `uuid` -- un `Cell::Text` (36 bytes de texto) bindeado directo contra un parámetro que el servidor infiere como `uuid` nativo (16 bytes binarios esperados) es un mismatch de wire. La solución, sin sumar una dependencia nueva (mismo criterio de siempre): `Db::id_placeholder` agrega `::uuid` al placeholder en cada SQL que liga un id contra una colección con PK Uuid corriendo sobre Postgres (`INSERT`, `WHERE "id" = $N`) -- el cast fuerza al servidor a inferir el parámetro como `unknown`/texto (que sí coincide con lo que el driver manda) y recién convertir a `uuid` DESPUÉS, con el parser de texto normal de Postgres. El mismo truco que prácticamente cualquier cliente sin tipo `uuid` nativo usa contra esta columna. SQLite nunca lo necesita -- no tiene un tipo binario separado del texto.
