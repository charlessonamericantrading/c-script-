...
**Verificado**: `map_pg_type_covers_the_common_scalar_types_without_a_warning` y las otras 4 pruebas de `introspect.rs` siguen pasando sin cambios (el mapeo por TIPO no se tocó); `introspect_warns_when_the_id_primary_key_is_not_actually_an_integer` (`pg_integration.rs`, nueva) confirma contra un PostgreSQL real que una tabla con `id UUID PRIMARY KEY DEFAULT gen_random_uuid()` sigue generando `id: Int` (nunca omite la columna) pero ahora con una advertencia que nombra la tabla, la columna y el tipo real.
