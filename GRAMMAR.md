...
**DDL: `CREATE UNIQUE INDEX IF NOT EXISTS "idx_<tabla>_<campo1>_<campo2>_..." ON "<tabla>"("<campo1>", "<campo2>", ...)` -- misma sentencia, válida en los DOS backends sin diferencias.** Mismo criterio de idempotencia y nombre determinístico que el `@unique` de un solo campo (§3.80): corre en CADA arranque, sin necesitar detectar "¿ya existía?". `--adopt-existing` nunca ejecuta este DDL, mismo criterio que el resto del schema. `linkc build` emite la sentencia estática en `schema.postgres.sql` (`codegen::postgres_emit.rs`, duplicado deliberado de la misma lógica que `runtime/db.rs::composite_unique_by_collection`/`create_composite_unique_statements` -- el emisor estático no instancia ningún `Db` real, mismo motivo que el resto del DDL estático de esa capa), y `linkc migrate --dry-run` (§3.97) lo incluye en su reporte.
