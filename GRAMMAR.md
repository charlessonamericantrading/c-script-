...
### 3.192 Bug real: `information_schema` hardcodeaba `'public'` (o no filtraba schema en absoluto) — RESUELTO

Encontrado investigando PLAN.md §9.3 ítem 4 (`--db-schema`, ver §3.193 más abajo) -- antes de diseñar la feature nueva, un research fork auditó las 8 consultas reales a `information_schema.*` de este proyecto contra el código actual (no contra la nota vieja de PLAN.md, que databa de antes de varias rondas recientes -- Decimal, `db shell`/`export`/`import`, `@requires` con dueño, `@encrypted` -- y ya no reflejaba con precisión el estado real). Resultado: **8 sitios en 3 archivos** (no "decenas" como la nota vieja temía), y **4 de los 8 no filtraban NINGÚN schema** -- un bug real, independiente de que `--db-schema` llegara a construirse o no, ya alcanzable hoy por cualquiera que conectara con un `search_path` propio (`options=` en la URL, o un rol Postgres con un `search_path` por default distinto de `public`).

**Los 8 sitios**: `runtime/db.rs::postgres_table_exists`/`validate_existing_id_column`/`warn_if_table_looks_unrelated`/`validate_columns_exist_for_adoption` (las cuatro detrás de `--adopt-existing`), `migrate.rs::existing_columns` (compartida por `linkc migrate --dry-run`, `db export`, y el loop de `ADD COLUMN` de la auto-migración), `introspect.rs::introspect_table` (dos consultas) y `generate_link_from_postgres` (una consulta, la que lista tablas). Cuatro (`postgres_table_exists` y las tres de `introspect.rs`) hardcodeaban `table_schema = 'public'` -- una tabla en cualquier OTRO schema era invisible, sin ningún error que lo explicara. Las otras cuatro no tenían filtro de schema en absoluto -- con más de un schema visible en el `search_path`, o dos tablas del mismo nombre en schemas distintos, podían leer la columna/tabla EQUIVOCADA en silencio.

**Fix: `table_schema = ANY(current_schemas(false))` en las 8, en vez de un nombre fijo o ningún filtro.** `current_schemas(boolean)` es una función nativa de Postgres que devuelve el `search_path` EFECTIVO de la sesión actual (`false` excluye schemas implícitos como `pg_catalog`) -- la MISMA fuente de verdad que el propio Postgres usa para resolver un identificador sin calificar (`CREATE TABLE "foo"` / `SELECT * FROM "foo"`, sin `"schema".` explícito). Con esto, las 8 consultas ahora ven exactamente lo mismo que vería cualquier otro SQL sin calificar contra la misma sesión -- `public` para casi cualquier conexión (el comportamiento de siempre, sin cambio observable), o lo que sea que la sesión tenga configurado.
