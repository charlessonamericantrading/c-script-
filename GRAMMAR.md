...
|---|---|---|
| `id: Int` | `INTEGER PRIMARY KEY` (no `AUTOINCREMENT`) | ver justificación abajo |
...
**`id` como `INTEGER PRIMARY KEY`, no `AUTOINCREMENT`.** En SQLite, `INTEGER PRIMARY KEY` es alias del rowid: insertar sin especificarlo autoasigna `max(rowid)+1`. `AUTOINCREMENT` solo agrega la garantía de "nunca reusar un id después de un borrado" -- irrelevante acá porque **no existe ningún método `delete` en todo el lenguaje**, así que esa situación no puede ocurrir por construcción. El esquema anterior (`rows.len()+1`) y `max(rowid)+1` son observacionalmente idénticos sin deletes; `AUTOINCREMENT` solo agregaría el overhead de una tabla `sqlite_sequence` sin ningún beneficio real.
...
- `delete`/`remove` -- no existe en el lenguaje hoy; esta ronda no agrega superficie CRUD nueva, solo persiste la que ya hay.
- Migraciones reales tipo `ALTER TABLE` -- ver arriba, falla fuerte en vez de auto-migrar.
- Índices más allá de `id` -- no hace falta ninguno hoy porque el lenguaje no tiene ningún mecanismo de query además de `find(id)`/`all()` + `.filter()` del lado interpretado.
