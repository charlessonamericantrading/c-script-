...
<div align="center">
  ...
  <p>
    ...
    <a href="#-testing--quality-assurance"><img src="https://img.shields.io/badge/tests-1319-success.svg" alt="Tests" /></a>
    <a href="https://github.com/charlessonamericantrading/c-script-/releases"><img src="https://img.shields.io/badge/versión-1.134.1-blue.svg" alt="Versión" /></a>
...
- `linkc introspect <url-postgres>` genera un `.link` de partida (types + `db {...}`) desde el schema de una base PostgreSQL ya existente -- para adoptar un sistema con datos reales en vez de escribir cada campo a mano. Punto de partida para revisar, no listo para producción tal cual: cualquier columna que no pueda mapear con confianza (`jsonb`, un `timestamp`/`timestamptz` nativo) igual sale con un tipo válido (`String`) más una advertencia en stderr, nunca se omite en silencio. Una clave primaria que se llama literalmente `id` emite `id: Int` para una columna entera o `id: Uuid` para una columna `uuid` nativa de Postgres, las dos sin advertencia -- cualquier otro tipo real sigue emitiendo el placeholder `id: Int` con una advertencia, porque esa forma todavía no es adoptable. Solo PostgreSQL, sin generar ningún `service`
