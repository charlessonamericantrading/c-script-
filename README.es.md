...
<div align="center">
  ...
  <p>
    ...
    <a href="#-testing--quality-assurance"><img src="https://img.shields.io/badge/tests-1302-success.svg" alt="Tests" /></a>
    <a href="https://github.com/charlessonamericantrading/c-script-/releases"><img src="https://img.shields.io/badge/versión-1.132.0-blue.svg" alt="Versión" /></a>
...
- `linkc introspect <url-postgres>` genera un `.link` de partida (types + `db {...}`) desde el schema de una base PostgreSQL ya existente -- para adoptar un sistema con datos reales en vez de escribir cada campo a mano. Punto de partida para revisar, no listo para producción tal cual: cualquier columna que no pueda mapear con confianza (`jsonb`, `uuid`, un `timestamp`/`timestamptz` nativo) igual sale con un tipo válido (`String`) más una advertencia en stderr, nunca se omite en silencio. Una clave primaria que se llama literalmente `id` sigue emitiendo el `id: Int` requerido, pero ahora avisa si la columna real no es un entero (por ejemplo `id uuid`) -- esa forma de PK igual se rechaza más adelante al conectar (`linkc serve`/`migrate --dry-run`), pero la advertencia lo muestra antes de escribir un `.link` entero alrededor de un placeholder que nunca fue real. Solo PostgreSQL, sin generar ningún `service`
