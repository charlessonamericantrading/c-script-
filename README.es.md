...
<div align="center">
  ...
  <p>
    ...
    <a href="#-testing--quality-assurance"><img src="https://img.shields.io/badge/tests-1410-success.svg" alt="Tests" /></a>
    <a href="https://github.com/charlessonamericantrading/c-script-/releases"><img src="https://img.shields.io/badge/versión-1.149.0-blue.svg" alt="Versión" /></a>
...
- `linkc doctor <archivo> [--db <url|archivo>] [--target-url <url>]`: diagnóstico de entorno antes de un despliegue -- la versión de `linkc`, que el archivo de entrada resuelva sus imports/parsee/tipe, permiso de escritura en su directorio, y conectividad de solo lectura (`SELECT 1`, nunca DDL) a la base configurada. Con `--target-url` (o `LINK_DOCTOR_TARGET_URL`), además consulta `<url>/health` de un `linkc serve` ya corriendo y avisa si la versión remota difiere de la local -- que el entorno de desarrollo quede atrás (o adelante) de producción es, si no, una deriva silenciosa sin ningún error que la señale. Imprime un checklist y sale con código `1` si algún chequeo real falló, pensado como paso de CI antes de `linkc serve`
...
- Un campo `Timestamp` ahora LEE y ESCRIBE columnas `date`/`timestamp`/`timestamptz` NATIVAS de PostgreSQL, no solo la convención `BIGINT`-milisegundos que genera `linkc build` -- el caso común al adoptar una tabla ya existente, donde las columnas de fecha casi siempre son el tipo nativo de Postgres. Codificado/decodificado a mano contra el wire binario de Postgres (sin sumar la dependencia `chrono`); `linkc introspect` ahora recomienda `Timestamp` sin advertencia para estas columnas, en vez de un mapeo a `String` que en la práctica tampoco funcionaba. La escritura era la mitad más peligrosa de este gap: antes del fix, `insert`/`applyPatch` contra una columna nativa guardaba en silencio la fecha equivocada (los dos formatos son binarios de 8 bytes, así que Postgres los aceptaba sin quejarse) en vez de fallar ruidosamente
...
- Adoptar una tabla con una columna `uuid` nativa de Postgres (un campo `Uuid` o `String` común, cualquiera de los dos) o `inet`/`cidr` (un campo `String`) ahora lee y escribe correctamente -- los tres tienen su propio formato binario de wire, no texto UTF-8, así que mapearlos a `String`/`Uuid` compilaba pero fallaba en la primera fila real (`linkc introspect` ya recomendaba `String` para `inet`; esa recomendación ahora funciona de verdad). Sin tipo nuevo -- una columna `source_ip inet` viaja sin problema a través de un campo común `source_ip: String?`
- `linkc introspect <url-postgres>` genera un `.link` de partida (types + `db {...}`) desde el schema de una base PostgreSQL ya existente -- para adoptar un sistema con datos reales en vez de escribir cada campo a mano. Punto de partida para revisar, no listo para producción tal cual: cualquier columna que no pueda mapear con confianza (`jsonb`, un `time` sin fecha) igual sale con un tipo válido (`String`) más una advertencia en stderr, nunca se omite en silencio. Una columna `uuid` nativa mapea a `Uuid`, y una `inet`/`cidr` nativa a `String`, las dos mapeos exactos sin advertencia -- leer/escribir esas columnas está verificado contra el formato binario real de Postgres, no solo "compila". Una clave primaria que se llama literalmente `id` emite `id: Int` para una columna entera o `id: Uuid` para una columna `uuid` nativa de Postgres, las dos sin advertencia -- cualquier otro tipo real sigue emitiendo el placeholder `id: Int` con una advertencia, porque esa forma todavía no es adoptable. Solo PostgreSQL, sin generar ningún `service`
- La clave primaria de una colección puede ser `id: Uuid` en vez de `id: Int` -- generada del lado de la aplicación en cada `insert` (mismo generador que `crypto.uuid()`), nunca depende de un `DEFAULT`/`RETURNING`/`last_insert_rowid()` de columna, que es justo lo que permite adoptar una tabla ya existente con `id uuid` (común al migrar desde otro backend) sin tocar su esquema. `find`/`applyPatch`/`delete`/`increment`/`insert`/`upsert`/`page`/`maxRow`/`minRow` funcionan igual que con `id: Int`. `pageAfter` queda rechazado a propósito sobre una colección con PK Uuid -- su garantía de "nunca se salta una fila insertada concurrentemente" depende de que el id crezca en el mismo orden que la inserción, algo que un UUID aleatorio no da; usá `page` (por offset) en su lugar. SQLite guarda la columna como `TEXT PRIMARY KEY NOT NULL`; PostgreSQL usa el tipo nativo `UUID`, con codificación/decodificación binaria hecha a mano para esa columna (sin sumar ninguna dependencia nueva) para que también funcione contra una columna `uuid` ya existente, no solo una que `linkc serve` creó
...
- Límite de requests por cliente: `@rate_limit("20/1m")` acota un rpc a N requests por ventana de tiempo, por `(ip del cliente, servicio, rpc)`, con 429 al exceder — token bucket con refill continuo. Sobre PostgreSQL, el bucket se comparte de verdad entre todas las instancias `linkc serve`/`serve-all` que apuntan a la misma base (una tabla interna, invisible para `db {}`/`introspect`/`migrate`) -- ya no multiplica el límite efectivo por instancia; se degrada solo al comportamiento por-proceso de siempre si la tabla no se puede crear (`--adopt-existing` sin crearla a mano antes, o un rol sin `CREATE TABLE`). SQLite sigue siendo por proceso -- un solo archivo/proceso ya tiene el estado exacto en memoria, nada que distribuir
...
- Las dependencias git de `link.json` (`"nombre": "git+<url>#<tag-o-rama-o-commit>"`) ahora fijan de verdad: después del primer `linkc build`, `link.lock` graba el commit resuelto y cada build/serve/test posterior reusa exactamente ese commit -- una dependencia por rama ya no se re-resuelve en silencio (antes de esta ronda, de hecho, quedaba congelada en el commit equivocado por un bug real de staleness en cómo se chequeaba el ref de la rama). `linkc build --update-deps` es el único comando que re-resuelve fresco y avanza el pin, mismo criterio que `cargo update` contra un `cargo build` normal. Dos `linkc build`/`serve` concurrentes resolviendo la misma dependencia ya no compiten por el mismo clon local tampoco (un lock advisory basado en archivo, que se autorepara si un proceso muere a mitad de un clone)
...
| Límite | Detalle |
|---|---|
| `@rate_limit` sobre SQLite es por proceso, en memoria | Sin persistencia entre reinicios, sin coordinación entre réplicas si el mismo `.link` corre en más de un proceso -- ver Funciona hoy para el caso PostgreSQL, que ahora comparte un bucket real entre todas las instancias que apuntan a la misma base. |
...
| Sin CSP | Depende del contenido real de cada página -- sin eso, no hay default seguro posible. (HSTS sí está disponible, opt-in: `--hsts <valor>`/`LINK_HSTS`, ya que `linkc serve` nunca termina TLS por sí mismo y no puede mandarlo por default -- ver Funciona hoy.) Las entradas del allowlist de CORS son match exacto únicamente, sin wildcards de subdominio. |
...
```bash
npx linkc --version          # una sola vez, sin instalar
npm install -g link-lang     # comando `linkc` global
```

`link-lang` es un instalador liviano, no el compilador en sí: la primera vez descarga el
`linkc` precompilado para tu plataforma desde el último
[GitHub Release](https://github.com/charlessonamericantrading/c-script-/releases), lo verifica
contra el `SHA256SUMS.txt` publicado y lo deja cacheado en `~/.c-script/bin`. Windows x64,
Linux x64, macOS Intel y Apple Silicon. O compilá desde el código:
...
El compilador y el runtime de Link están verificados por **1410 pruebas automáticas** unitarias,
