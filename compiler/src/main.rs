...
fn main() -> ExitCode {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("triggers") => cmd_triggers(&args[2..]),
...
/// GRAMMAR.md §3.241: `--max-concurrency <N>`/`LINK_MAX_CONCURRENCY` -- un
/// entero >= 1; ausente = sin tope (la conducta de siempre).
fn resolve_max_concurrency(args: &[String]) -> Result<Option<usize>, String> {
    let Some(raw) = read_flag_or_env(args, "--max-concurrency", "LINK_MAX_CONCURRENCY")? else {
        return Ok(None);
    };
    match raw.trim().parse::<usize>() {
        Ok(n) if n >= 1 => Ok(Some(n)),
        _ => Err(format!("--max-concurrency/LINK_MAX_CONCURRENCY: se esperaba un entero >= 1, se recibió '{raw}'")),
    }
}

/// PLAN.md §9.20 Fase 1.1 / GRAMMAR.md §3.244: `--db-pool-size <N>`/`LINK_DATABASE_POOL_SIZE`
/// -- tamaño del pool de conexiones PostgreSQL (entero >= 1; ausente = 10 por default).
fn resolve_db_pool_size(args: &[String]) -> Result<Option<usize>, String> {
    let Some(raw) = read_flag_or_env(args, "--db-pool-size", "LINK_DATABASE_POOL_SIZE")? else {
        return Ok(None);
    };
    match raw.trim().parse::<usize>() {
        Ok(n) if n >= 1 => Ok(Some(n)),
        _ => Err(format!("--db-pool-size/LINK_DATABASE_POOL_SIZE: se esperaba un entero >= 1, se recibió '{raw}'")),
    }
}

/// GRAMMAR.md §3.238: `--fallback-upstream <url>`/`LINK_FALLBACK_UPSTREAM`
/// -- validado al arrancar: `http://` o `https://`, sin path (el path lo
/// pone cada request), para que un valor mal escrito no se descubra en el
/// primer 404 en producción.
fn resolve_fallback_upstream(args: &[String]) -> Result<Option<String>, String> {
    let Some(raw) = read_flag_or_env(args, "--fallback-upstream", "LINK_FALLBACK_UPSTREAM")? else {
        return Ok(None);
    };
    let value = raw.trim().trim_end_matches('/').to_string();
    if !(value.starts_with("http://") || value.starts_with("https://")) {
        return Err(format!("--fallback-upstream/LINK_FALLBACK_UPSTREAM: se esperaba una URL http(s)://host[:puerto], se recibió '{raw}'"));
    }
    let rest = value.split("://").nth(1).unwrap_or("");
    if rest.is_empty() || rest.contains('/') || rest.contains('?') {
        return Err(format!(
            "--fallback-upstream/LINK_FALLBACK_UPSTREAM: solo esquema, host y puerto (el path lo pone cada request), se recibió '{raw}'"
        ));
    }
    Ok(Some(value))
}
...
/// `linkc triggers <archivo.link> [--db-schema <nombre>]` (GRAMMAR.md
/// §3.225): imprime a stdout el DDL idempotente de PostgreSQL que hace que
/// un `stream` reaccione a escrituras externas. Solo texto: no se conecta a
/// ninguna base ni aplica nada -- quien administra la base lo revisa y lo
...
fn cmd_introspect(args: &[String]) -> ExitCode {
    let Some(url) = args.first() else {
        eprintln!("uso: linkc introspect <db-url|archivo.db>");
        return ExitCode::FAILURE;
    };
    match linkc::introspect::generate_link(url) {
        Ok((content, warnings)) => {
            println!("{content}");
            if !warnings.is_empty() {
...
fn cmd_serve(args: &[String]) -> ExitCode {
    ...
    let rate_limit_global = match resolve_rate_limit_global(args) {
        Ok(v) => v,
        Err(msg) => {
            eprintln!("{msg}");
            return ExitCode::FAILURE;
    ...
    let drain_timeout = match resolve_drain_timeout(args) {
        Ok(v) => v,
        Err(msg) => {
            eprintln!("{msg}");
            return ExitCode::FAILURE;
    ...
    // C14): UN manejador de señal para todo el proceso -- `ctrlc::
    // set_handler` solo se puede llamar una vez, y `linkc serve` (a
    // diferencia de `serve-all`) corre un solo servicio, así que esto es
    // seguro acá. El handler en sí solo fija el flag -- el drenado real
    // corre en el loop principal de `runtime::server::serve`.
    let draining = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));
    {
        ...
        if let Err(e) = ctrlc::set_handler(move || {
            ...
            eprintln!("no se pudo instalar el manejador de señal para el drenado gracioso: {e}");
            return ExitCode::FAILURE;
        }
    }
    ...
    let fallback_upstream = match resolve_fallback_upstream(args) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("{e}");
            return ExitCode::FAILURE;
    ...
    let max_concurrency = match resolve_max_concurrency(args) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("{e}");
            return ExitCode::FAILURE;
    ...
    let db_pool_size = match resolve_db_pool_size(args) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("{e}");
            return ExitCode::FAILURE;
...
/// GRAMMAR.md §3.281 (PLAN.md §9.24 Fase 1 ítem C8a): `--rate-limit-global`/
/// `LINK_RATE_LIMIT_GLOBAL` -- un tope para TODO el sitio (una sola clave
/// por IP, sin importar qué rpc golpee), a diferencia de `@rate_limit`
/// (por rpc). Mismo formato "N/ventana" que `@rate_limit`, reusa el mismo
/// parser (`RateLimitSpec::parse`) -- dos gramáticas para la misma idea
/// sería exactamente la clase de divergencia entre capas que este proyecto
/// viene evitando desde GRAMMAR.md §3.9. Sin el flag: `None`, ningún tope
/// global -- comportamiento IDÉNTICO al de siempre (el único límite sigue
/// siendo el de cada `@rate_limit` individual, si el programa declara
/// alguno).
fn resolve_rate_limit_global(args: &[String]) -> Result<Option<linkc::rate_limit::RateLimitSpec>, String> {
    match read_flag_or_env(args, "--rate-limit-global", "LINK_RATE_LIMIT_GLOBAL")? {
        None => Ok(None),
        Some(raw) => linkc::rate_limit::RateLimitSpec::parse(&raw).map(Some).map_err(|e| format!("--rate-limit-global/LINK_RATE_LIMIT_GLOBAL: {e}")),
    }
}
...
fn cmd_serve_all(args: &[String]) -> ExitCode {
    // GRAMMAR.md §3.238: mismo upstream para todos los servicios de
    // `serve-all` (el backend viejo es uno solo).
    let fallback_upstream = match resolve_fallback_upstream(args) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("{e}");
    ...
    // GRAMMAR.md §3.241: por servicio (cada uno tiene su propio pool de hilos).
    let max_concurrency = match resolve_max_concurrency(args) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("{e}");
    ...
    // GRAMMAR.md §3.282: UN manejador de señal para TODO el proceso
    // `serve-all` -- `ctrlc::set_handler` solo se puede llamar una vez, y
    // acá corren VARIOS servicios en hilos separados del MISMO proceso, así
    // que el flag se comparte (mismo `Arc`) entre todos: una sola señal
    // drena a todos a la vez.
    let draining = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));
    {
        let draining = std::sync::Arc::clone(&draining);
        if let Err(e) = ctrlc::set_handler(move || {
...
/// GRAMMAR.md §3.282 (PLAN.md §9.18 Eje E ítem 1 / §9.24 Fase 1 ítem C14):
/// `--drain-timeout`/`LINK_DRAIN_TIMEOUT` -- cuánto espera el drenado
/// gracioso a que las requests en vuelo terminen solas antes de salir de
/// todos modos. 10s por default -- PLAN.md ya lo proponía como default;
/// mismo parser que `--http-timeout`/`--session-ttl` (`parse_duration`).
fn resolve_drain_timeout(args: &[String]) -> Result<std::time::Duration, String> {
    let raw = read_flag_or_env(args, "--drain-timeout", "LINK_DRAIN_TIMEOUT")?;
    let Some(raw) = raw else {
