...
pub fn get_completions(source: &str, line0: usize, col0: usize, full_program: Option<&Program>) -> Vec<Value> {
    ...
    if prefix.trim_end().ends_with('.') {
        ...
        let methods = [
            ...
            ("updateWhere(fn, patch)", "Update records matching a predicate", 2),
            ("findWhere(fn)", "Find records matching a predicate", 2),
            ("select(fn)", "Select projected fields or scalar from records", 2),
...
fn completions_for_receiver_type(ty: &Type) -> Option<Vec<Value>> {
    ...
    match ty {
        Type::DbCollection(_) => Some(vec![
            ...
            method("updateWhere(fn, patch)", "Update records matching a predicate"),
            method("findWhere(fn)", "Find records matching a predicate"),
            method("select(fn)", "Project fields directly in SQL (or memory fallback)"),
            method("subscribe()", "Subscribe to live changes in a stream"),
            method("orderBy(fn)", "Order by a field, ascending (then .all()/.page()/.findWhere()/.select())"),
            method("orderByDesc(fn)", "Order by a field, descending (then .all()/.page()/.findWhere()/.select())"),
        ]),
        ...
        Type::DbQuery(_) => Some(vec![
            ...
            method("select(fn)", "Run the ordered query with projected fields"),
