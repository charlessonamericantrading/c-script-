...
impl MetricsStore {
    ...
    /// colección) y `db_size_bytes` vienen de `Db` (lo único que los sabe),
    /// pasados por el caller en vez de que este módulo dependa de `db.rs`.
    pub fn render_prometheus_text(&self, stream_subscribers: &[(String, usize)], db_size_bytes: Option<i64>) -> String {
