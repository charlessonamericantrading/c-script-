...
// examples/frontend/).
//
// `stream` (GRAMMAR.md §2.1, streaming real v0): un hilo POR CONEXIÓN de
// streaming, spawneado desde el loop principal -- así una respuesta larga
// no bloquea al servidor de aceptar el resto de las conexiones. El loop
// principal en sí sigue siendo estrictamente secuencial para `rpc`
// normales (igual que antes), evitando cualquier necesidad de sincronizar
// ese camino.
...
/// stdout al mismo tiempo (cada `stream` corre en el suyo). No es una
/// iniciativa de observabilidad aparte: es lo mínimo que el cambio a
/// multi-hilo hace necesario, ver PLAN.md §4 (Fase 2).
...
pub fn serve(program: Program, port: u16) {
    ...
    // Arc, no una referencia con lifetime: cada conexión de streaming
    // spawnea su PROPIO hilo (más abajo) que necesita su propia ownership
    // handle sobre el programa y la db, viva más allá de esta iteración del
    // loop. Program/Db no tienen mutabilidad interior propia salvo los
    // Mutex<Vec<Value>> ya dentro de Db, así que Arc<Program>/Arc<Db> son
    // Send+Sync automáticos -- no hace falta ningún cambio de tipos interno.
    let program = Arc::new(program);
    let db = Arc::new(Db::seeded());
    ...
    for mut request in server.incoming_requests() {
        ...
        if is_stream_member(&program, service_name, rpc_name) {
            ...
            // `Env` (runtime/mod.rs, Rc<RefCell<Value>>) no es Send -- pero
            // nunca cruza este borde: invoke_rpc la arma y la descarta
            // enteramente DENTRO de serve_stream, que corre ya en el hilo
            // nuevo. Lo único que efectivamente se mueve acá es Arc<Program>,
            // Arc<Db> y un puñado de String -- todo Send.
            std::thread::spawn(move || {
                serve_stream(request, &program, &db, &service_name, &rpc_name, &body, req_id);
            });
            continue;
        }
...
/// Corre enteramente en su propio hilo. Alcance v0 explícito (GRAMMAR.md
/// §2.1, PLAN.md §4): repite una secuencia YA CALCULADA -- invoke_rpc
/// evalúa el cuerpo COMPLETO primero (igual que un rpc normal, el checker
/// ya exige que ese cuerpo sea List<T>), y recién con la lista entera en
/// mano se abre la respuesta y se emite un evento SSE por elemento. No hay
/// generación perezosa ni suscripción a cambios futuros -- ninguna de las
/// dos es posible todavía (el lenguaje no tiene ningún constructo de loop,
/// y "suscribirse" necesitaría una capa de pub-sub sobre Db que no existe).
fn serve_stream(
